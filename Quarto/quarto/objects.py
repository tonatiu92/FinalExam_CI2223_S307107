...
class Piece(object):
    ...
    def __str__(self):
        to_str = f"HIGH: {self.HIGH}, COLOURED: {self.COLOURED},SOLID: {self.SOLID},SQUARE: {self.SQUARE},"
        return to_str
...
class Quarto(object):
    ...
    def __init__(self) -> None:
        ...
        self.__pieces = []
        self.__pieces.append(Piece(False, False, False, False))  # 0 little, not coloured, hollow, not square
        self.__pieces.append(Piece(False, False, False, True))  # 1
        self.__pieces.append(Piece(False, False, True, False))  # 2
        self.__pieces.append(Piece(False, False, True, True))  # 3 antagonist 12
        self.__pieces.append(Piece(False, True, False, False))  # 4 antagonist 11
        self.__pieces.append(Piece(False, True, False, True))  # 5 antagonist 10
    ...
    def run(self) -> int:
        ...
        while winner < 0 and not self.check_finished():
            #print(f'Turn n° {i-1} of PLAYER {self.__current_player} Choosing')
            #self.print()
            piece_ok = False
            while not piece_ok:
                piece_ok = self.select(self.__players[self.__current_player].choose_piece())
            piece_ok = False
            #print(f"PLAYER {self.__current_player} choosed n°{self.__selected_piece_index}")
            self.__current_player = (self.__current_player + 1) % self.MAX_PLAYERS
            #print(f'Turn n° {i-1} of PLAYER {self.__current_player} Placing')
            while not piece_ok:
